...
opts,args=getopt.getopt(sys.argv[1:],'i:',['ref=','bam=','ppre=','sv=','Path_Lumpy=','Path_Delly='])
dict_opts=dict(opts)
start=0
window_size=10
  ...
  if not k1 in ['chrX','chrY']:
    for k2 in comparison[k1]:
        ...
        if flag_run==0:
            ...
            #SVelter_Stat=numpy.mean(SVelter_Info[1])
            #if not Delly_Info[0]==[]:
              #Delly_Stat=numpy.mean([numpy.mean(Delly_Info[1]),numpy.mean(Delly_Info[2])])
             # Delly_Stat=numpy.mean(Delly_Info[1])
            #else:
             # Delly_Stat=1
            #if not Lumpy_Info[0]==[]:
              #Lumpy_Stat=numpy.mean([numpy.mean(Lumpy_Info[1]),numpy.mean(Lumpy_Info[2])])
             # Lumpy_Stat=numpy.mean(Lumpy_Info[1])
            #else:
             # Lumpy_Stat=1
            ...
            if not Lumpy_Info[0]==[]:
              ...
              Lumpy_Stat=1
            print >>fo, ' '.join([str(i) for i in ['_'.join([str(j) for j in [k1]+k2[0][0][:4]])]+[SVelter_Stat,Delly_Stat,Lumpy_Stat]]+['1'])

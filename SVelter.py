        ...
        if not '--WorkDir' in dict_opts.keys():
            ...
            if not '--Reference' in dict_opts.keys():
                ...
                if not os.path.isfile(ref_index):
                    ...
                    if not '--SVelterPath' in dict_opts.keys():
                        print 'Error: please specify path of SVelter scripts using --SVelterPath'                       
                    else:
...
if function_name=='NullModel':
    ...
    def IL_Stat_Calculate(InsertLength):
        ...
        #Min_keys=0
        #Max_keys=0
        #for x in sorted(InsertLength.keys()):
        #    Min_keys+=InsertLength[x]
        #    if float(Min_keys)/float(TotalILNum)<0.1:
        #        continue
        #    else:
        #        break
        #for x in sorted(InsertLength.keys())[::-1]:
        #    Max_keys+=InsertLength[x]
    ...
    def clean_files():
        ...
        #if os.path.isfile(SplitNullfigure1):
        #    os.system('''mv %s %s'''%(SplitNullfigure1,NullPath+'Split_Null'))
        #if os.path.isfile(SplitNullfigure2):
        ...
        #if os.path.isfile(outputfileIL):
        #    os.system('''mv %s %s'''%(outputfileIL,NullPath+'All_Stats'))
        #if os.path.isfile(outputfileStat):
                        ...
                        for bamF in bam_files:
                            ...
                            for chrF in chromos:
                                ...
                                if os.path.isfile(Refloc_name):
                                    ...
                                    #if float(SPLCff)/float(ReadLength)>0.2:
                                    #    SPCluMin=3
                                    ...
                                    #if float(ILStats['stat']['Mean'])/float(ILStats['s tat']['STD'])<2: 
        ...
        if not '--WorkDir' in dict_opts.keys():
            ...
            if not '--File' in dict_opts.keys():
                ...
                if not '-o' in dict_opts.keys():
                    dict_opts['-o']='/'.join(dict_opts['--File'].split('/')[:-1])
                ...
                if not '--Sample' in dict_opts.keys():
                    ...
                    output_Score_File=dict_opts['-o']+'_'.join(dict_opts['--File'].split('/')[-1].split('.')[:-1]+['flag'+str(Ploidy)])+'_'+'MP'+str(QCAlign)+'.coverge'

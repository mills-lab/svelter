...
#!python
#command='python SVelter.vcf2BPvcf.py --ref /nfs/remills-scratch/datasets/Simulation.Xuefang/reference.flux/human_g1k_v37.fasta -p /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.comp/Lumpy/ --prefix Lumpy_comp_RD_10_pesr'
...
for k1 in os.listdir(dict_opts['-p']):
	if dict_opts['--prefix'] in k1 and k1.split('.')[-1]=='bedpe':
...
for line in fin:
	pin=line.strip().split()
	pos=[pin[0],int(numpy.mean([int(pin[1]),int(pin[2])])),int(numpy.mean([int(pin[4]),int(pin[5])]))]
	if pos[2]-pos[1]>99:
		gt='het'
		...
		if not k2 in sv_hash[k1].keys():
			sv_hash[k1][k2]=[]
		sv_hash[k1][k2].append(pos+[0.0,pin[6]])

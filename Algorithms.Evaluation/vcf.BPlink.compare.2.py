...
#command='python vcf.BPlink.compare.py --in /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.comp/SVelter/SVelter_comp_RD60.BPlink.vcf --ref /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.comp/sv.rec/comp.vcf'
...
def hash_name_link(ref_order,in_order):
	...
	for k1 in link_hash.keys():
		...
		for k2 in link_hash[k1].keys():
			...
			if not link_hash[k1][k2]==[]:
				...
				score_spec[k1].append(max([i[1] for i in temp_score])/len(in_d2))
			else:
	...
	for k1 in score_sens.keys():
		...
		score_spec[k1]=numpy.mean(score_spec[k1])		
		score_allnum[k1]=numpy.sum(score_allnum[k1])
	return [score_sens,score_spec,score_allnum,score_Not_missed]
...
for k1 in out[0].keys():
	print >>fo, ' '.join([filein.split('/')[-1].split('.')[0],k1,str(out[0][k1]),str(float(out[1][k1])),str(out[2][k1]),str(out_ref[k1]),str(out_in[k1])])
	print 		' '.join([filein.split('/')[-1].split('.')[0],k1,str(out[0][k1]),str(float(out[1][k1])),str(out[2][k1]),str(out_ref[k1]),str(out_in[k1])])

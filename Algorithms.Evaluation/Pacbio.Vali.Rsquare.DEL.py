    ...
    for line in fbam:
        ...
        if not pbam[0]=='@': 
            if int(pbam[3])<int(bps[1])-flank_length+1:
                ...
                if len(target_read)>flank_length+int(bps[2])-int(bps[1]):
                    out.append(target_read[:int(bps[2])-int(bps[1])+2*flank_length])
                    out2.append(miss_bp)
                ...
                target_read=pbam[9][align_start:]
                if len(target_read)>int(bps[2])-int(bps[1]):
                    out.append(target_read[:int(bps[2])-int(bps[1])+flank_length])
                    out2.append(miss_bp)
    fbam.close()
    return [out,out2]

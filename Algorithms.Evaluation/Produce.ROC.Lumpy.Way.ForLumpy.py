...
#!Python
#command='Produce.ROC.Lumpy.Way.ForLumpy.py --path_in /mnt/EXT/Mills-scratch/datasets/Simulation.Xuefang/Simulate.het/Lumpy/bedpe/ --path_ref /mnt/EXT/Mills-scratch/datasets/Simulation.Xuefang/Simulate.het/sv_new.rec/bed_files --appdix .Mappable.TRAFree.min100.max1000000000.bed'
...
for k1 in os.listdir(in_path):
	if dict_opts['--appdix']+'pe' in k1:
		in_hash.append(in_path+k1)

out_stat={}
for k1 in in_hash:
	if os.path.isfile(ref_path+k1.split('/')[-1][:-2]):
		file1=k1
		file2=ref_path+k1.split('/')[-1][:-2]
			...
			if ka in ref_hash.keys():
				...
				for kb in ref_hash[ka]:
					...
					for kc in data_hash[ka]:
						if abs(kc[0]-kb[0])<50 or abs(kc[1]-kb[0])<50:
							if abs(kc[2]-kb[1])<50 or abs(kc[3]-kb[1])<50:
								flag1+=1
						if not flag1==0:
							break
					flag2+=flag1

#command='python Delly.vcf2BPvcf.py --ref //nfs/remills-scratch/datasets/Simulation.Xuefang/reference.flux/human_g1k_v37.fasta --prefix /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.comp/Delly/Delly_comp_RD20'
...
dict_opts=dict(opts)
prefix=dict_opts['--prefix']
path='/'.join(prefix.split('/')[:-1])+'/'
filesIn=[]
for k1 in os.listdir(path):
	if prefix.split('/')[-1] in k1 and k1.split('.')[-1]=='vcf':
		filesIn.append(k1)
...
for filein in filesIn:
	if 'DEL' in filein or 'del' in filein:
		fin=open(path+filein)
		k1='a/a'
		for line in fin:
			pin=line.strip().split()
			if not pin[0][0]=='#':
				if pin[6]=='PASS':
					...
					if not k2 in sv_hash[k1].keys():
						sv_hash[k1][k2]=[]
					sv_hash[k1][k2].append(pos+[0])
		fin.close()
	elif 'INV' in filein or 'inv' in filein:
		fin=open(path+filein)
		k1='a/a'
		for line in fin:
			pin=line.strip().split()
			if not pin[0][0]=='#':
				if pin[6]=='PASS':
					...
					sv_hash[k1][k2].append(pos+[0])
		fin.close()
	elif 'DUP' in filein or 'dup' in filein:
		fin=open(path+filein)
		k1='a/a'
		for line in fin:
			pin=line.strip().split()

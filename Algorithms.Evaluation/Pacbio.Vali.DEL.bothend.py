...
#bps on both side will be checked
import os
import sys
import getopt
import re
...
def remove_files(txt_file):
    ...
    if os.path.isfile(txt_file.replace('.txt','*.fa')):
            ...
            if bps_check(bps)==0:
                ...
                if int(bps[-1])-int(bps[1])<100:
                    ...
                    if int(bps[-1])-int(bps[1])<500:
                        ...
                        flank_length=500
                all_reads_left=chop_pacbio_read_left()
                all_reads_right=chop_pacbio_read_right()
                ...
                if not read_hash_left==[]:
                    miss_rec=-1
                    rec_len=0
                    if len(read_hash_left)>50:
                        read_num_sample=random.sample_name(range(len(read_hash_left)),50)
                    ...
                    for x in read_hash_left:
                        ...
                        if not rsquare_alt1==[]:
                            if max([rsquare_alt1[-1],rsquare_alt2[-1]])-rsquare_ref[-1]>rec_len:
                                ...
                                rec_start=y2
                                os.system(r'''cp %s %s'''%(fo_ref+'.txt',fo_ref+'.left.longest'))
                                os.system(r'''cp %s %s'''%(fo1_alt+'.txt',fo1_alt+'.left.longest'))
                                os.system(r'''cp %s %s'''%(fo2_alt+'.txt',fo2_alt+'.left.longest'))
                                os.system(r'''cp %s %s'''%(fa1,txt_file.replace('.txt','.left.sample.fa')))                                
                    if os.path.isfile(txt_file.replace('.txt','.left.sample.fa')):
                        os.system(r'''dotplot.py %d %s %s %s'''%(window_size,txt_file.replace('.txt','.left.sample.fa'),fa2,fo_ref+'.left.longest.png'))
                        os.system(r'''dotplot.py %d %s %s %s'''%(window_size,txt_file.replace('.txt','.left.sample.fa'),fa3,fo1_alt+'.left.longest.png'))
                        os.system(r'''dotplot.py %d %s %s %s'''%(window_size,txt_file.replace('.txt','.left.sample.fa'),fa4,fo2_alt+'.left.longest.png'))
                        os.system(r'''mv %s %s'''%(fo_ref+'.left.longest',fo_ref+'.left.longest.'+str(rec_start)))
                        os.system(r'''mv %s %s'''%(fo1_alt+'.left.longest',fo1_alt+'.left.longest.'+str(rec_start)))
                        os.system(r'''mv %s %s'''%(fo2_alt+'.left.longest',fo2_alt+'.left.longest.'+str(rec_start)))
                if not read_hash_right==[]:
                    miss_rec=-1
                    rec_len=0
                    if len(read_hash_right)>50:
                        read_num_sample=random.sample_name(range(len(read_hash_right)),50)
                    ...
                    for x in read_hash_right:
                        ...
                        if not rsquare_alt1==[]:
                            if max([rsquare_alt1[-1],rsquare_alt2[-1]])-rsquare_ref[-1]>rec_len:
                                rec_len=max([rsquare_alt1[-1],rsquare_alt2[-1]])-rsquare_ref[-1]
                                rec_start=y2
                                os.system(r'''cp %s %s'''%(fo_ref+'.txt',fo_ref+'.right.longest'))
                                os.system(r'''cp %s %s'''%(fo1_alt+'.txt',fo1_alt+'.right.longest'))
                                os.system(r'''cp %s %s'''%(fo2_alt+'.txt',fo2_alt+'.right.longest'))
                                os.system(r'''cp %s %s'''%(fa1,txt_file.replace('.txt','.right.sample.fa')))                                
                    if os.path.isfile(txt_file.replace('.txt','.right.sample.fa')):
                        os.system(r'''dotplot.py %d %s %s %s'''%(window_size,txt_file.replace('.txt','.right.sample.fa'),fa2.replace('.fa','.reverse.fa'),fo_ref+'.right.longest.png'))
                        os.system(r'''dotplot.py %d %s %s %s'''%(window_size,txt_file.replace('.txt','.right.sample.fa'),fa3.replace('.fa','.reverse.fa'),fo1_alt+'.right.longest.png'))
                        os.system(r'''dotplot.py %d %s %s %s'''%(window_size,txt_file.replace('.txt','.right.sample.fa'),fa4.replace('.fa','.reverse.fa'),fo2_alt+'.right.longest.png'))
                        os.system(r'''mv %s %s'''%(fo_ref+'.right.longest',fo_ref+'.right.longest.'+str(rec_start)))
                        os.system(r'''mv %s %s'''%(fo1_alt+'.right.longest',fo1_alt+'.right.longest.'+str(rec_start)))
                        os.system(r'''mv %s %s'''%(fo2_alt+'.right.longest',fo2_alt+'.right.longest.'+str(rec_start)))

...
#!python
#command='python Report.BPLink.Compare.py --ref /nfs/remills-scratch/datasets/Simulation.Xuefang/reference.flux/human_g1k_v37.fasta -i /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.het/TRA.Compare.Report/Delly_het_RD_10_comp.SV.compare.report'
...
import time
import datetime
opts,args=getopt.getopt(sys.argv[1:],'i:o:h:',['ref=','prefix=','appdix='])
dict_opts=dict(opts)
filesIn=dict_opts['-i']
ref=dict_opts['--ref']
chromos=[]
	...
	if k1 in tra_samp.keys():
		...
		for kx in tra_ref2[k1].keys():
			for ky in tra_samp[k1].keys():
				comp_score=0
				for km in tra_ref2[k1][kx]:
					for kn in tra_samp[k1][ky]:
						if km[0]==kn[0] and abs(km[1]-kn[1])<100:
							if descript_compare(km[-1],kn[-1])==1:
								comp_score+=1
				compScores.append([comp_score,len(tra_ref2[k1][kx]),len(tra_samp[k1][ky])])
		compScores2=[]
		for x in range(len(compScores)/2):
			if compScores[2*x][0]>compScores[2*x+1][0]:
				compScores2.append(compScores[2*x])
			else:
				compScores2.append(compScores[2*x+1])
		compare[k1]=compScores2
...
for k1 in tra_ref2.keys():
	if k1 in tra_samp.keys():
		common_keys.append(k1)

fout=open(filesIn.replace('.report','.total.Stat'),'w')
print >>fout, ' '.join([str(i) for i in [len(common_keys),len(tra_ref2.keys()),len(tra_samp.keys())]])
fout.close()
...
fout=open(filesIn.replace('.report','.2.Stat'),'w')
for k1 in compare.keys():
	for k2 in compare[k1]:
		print >>fout, ' '.join([str(i) for i in k2]+[k1]+['_'.join(i) for i in hash_test[k1]])

#command='python /mnt/EXT/Mills-data/xuefzhao/code/Code14/Code5.StructureResolvation2.py  --ref /nfs/remills-scratch/datasets/Simulation.Xuefang/reference.flux/human_g1k_v37.fasta --ppre /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.het/ --RSPath /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.het/bp_files/Simulate.het.RD.20.RL.101.sorted/SPCff4.CluCff6.AlignCff0.0 -o /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.het/SVelter/test.vcf'
#command='python CSV.rec2vcf.py -i comp.het.comp.SV.rec --ref /nfs/remills-scratch/datasets/Simulation.Xuefang/reference.flux/human_g1k_v37.fasta'
...
import time
import datetime
opts,args=getopt.getopt(sys.argv[1:],'i:o:h:',['ref='])
dict_opts=dict(opts)
...
for line in fin:
	pin=line.strip().split()
	if not pin[-2] in sv_info.keys():
		sv_info[pin[-2]]={}
	if not pin[-1] in sv_info[pin[-2]].keys():
		sv_info[pin[-2]][pin[-1]]=[]
	sv_info[pin[-2]][pin[-1]].append(pin[:-2]+[0])
...
score_Cff=-50
hash_reorder()
output_file='/'.join(filein.split('/')[:-1]+[filein.split('/')[-1].replace('.rec','.vcf')])

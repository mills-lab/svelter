#command='python SVelter.vcf2BPvcf.py --ref /mnt/EXT/Mills-scratch/datasets/Simulation.Xuefang/reference/human_g1k_v37.fasta -i /mnt/EXT/Mills-scratch/datasets/Simulation.Xuefang/Simulate.comp/SVelter/Simulate.comp.RD.10.RL.101.vcf'
...
def tra_info_add_new(k1,k2):
	for k3 in sv_info[k1][k2]:
		...
		SV_ID='_'.join(k3[-1].split('_')[:-1]+[k3[-2],k3[-1].split('_')[-1]])
		tra1[SV_ID]={}
		k2a=k2.split('/')[0]
		k2b=k2.split('/')[1]
		bp_hash={}
		...
		if pin[6]=='PASS':
			...
			if pin[4]=='<DEL>':
				...
				if not k2 in sv_hash[k1].keys():
					sv_hash[k1][k2]=[]
				sv_hash[k1][k2].append(pos+[float(pin[5])]+[pin[2]])
			elif pin[4]=='<INV>':
				k1='a/a'
				if gt=='het':
					k2='a/a^'
				...
				k1='a/a'
				if gt=='het':
					k2='a/'+''.join(['a' for i in range(CopyNum)])
				elif gt=='homo':
					k2=''.join(['a' for i in range(CopyNum)])+'/'+''.join(['a' for i in range(CopyNum)])
				if not k1 in sv_hash.keys():
					sv_hash[k1]={}
				if not k2 in sv_hash[k1].keys():
					sv_hash[k1][k2]=[]
...
hash_reorder_new()
hash_add()
output_file=dict_opts['-i'].replace('.vcf','.BPLink.vcf')

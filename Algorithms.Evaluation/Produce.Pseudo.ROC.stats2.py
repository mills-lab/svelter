...
#command='python SVelter.python --path_ref path1 --path_in path2 --appdix appdix'
import os
import sys
import getopt
import numpy
...
def hash_compare(ref_bed,samp_bed):	
    for k1 in ref_bed.keys():
        if k1 in ref_bed.keys() and k1 in samp_bed.keys():
            ...
            for k2 in ref_bed[k1]:
                ...
                for k3 in samp_bed[k1]:
                    if k3[1]<k2[0]:
                        ...
                        if not float(sorted(k2+k3)[2]-sorted(k2+k3)[1])/max([float(k2[1]-k2[0]),float(k3[1]-k3[0])])<0.8:
                            flag2+=1
                if flag2>0:
                    flag1+=1
                else:
            ...
            print [k1,flag1,float(flag1)/float(len(ref_bed[k1]))]

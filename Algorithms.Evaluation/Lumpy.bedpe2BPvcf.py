...
#command='python Lumpy.bedpe2BPvcf.py --ref /nfs/remills-scratch/datasets/Simulation.Xuefang/reference.flux/human_g1k_v37.fasta --prefix /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.comp/Lumpy/Simulate.comp.RD.10.RL.101'
...
for k1 in os.listdir(path):
	if prefix.split('/')[-1] in k1 and k1.split('.')[-1]=='bedpe':
		filesIn.append(k1)
...
for filein in filesIn:
	...
	for line in fin:
		...
		if 'DEL' in pin[10]:
			...
			k1='a/a'
			k2='a/a^'
		pos=[pin[0],int(numpy.mean([int(i) for i in pin[1:3]])),int(numpy.mean([int(i) for i in pin[4:6]]))]

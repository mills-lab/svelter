...
#!python
#command='python vcf2BPvcf.rec.py --ref /nfs/remills-scratch/datasets/Simulation.Xuefang/reference.flux/human_g1k_v37.fasta -i /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.comp/comp.SV.rec'
...
import time
import datetime
opts,args=getopt.getopt(sys.argv[1:],'i:',['ref=','prefix=','appdix='])
dict_opts=dict(opts)
...
for line in fin:
	...
	if not pin[-1] in sv_info[pin[-2]].keys():
		sv_info[pin[-2]][pin[-1]]=[]
	sv_info[pin[-2]][pin[-1]].append(pin[:-2])
...
sv_hash2={}
hash_add()
output_file=dict_opts['-i']+'.BPLink.vcf'

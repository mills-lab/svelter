...
#!python
#command='Comp.Compare.SVelter.py --ref_genome /nfs/remills-scratch/datasets/Simulation.Xuefang/reference.flux/human_g1k_v37.fasta -p /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.comp/bp_files.Simulate.comp.RD.10.RL.101.sorted.bam/Simulate.comp.RD.10.RL.101.sorted/SPCff3.CluCff3.AlignCff0.0 --ref /nfs/remills-scratch/datasets/Simulation.Xuefang/Simulate.comp/comp.SV.rec'
...
def write_comp_new(comp_new,fout):
	#fout=dict_opts['-p']+'SVelter.SV.compare.report'
	fo=open(fout,'w')
	for x in comp_new.keys():
		for y in comp_new[x].keys():
			for z in comp_new[x][y]:
...
fin.close()
chromos=chromos_name_readin(dict_opts['--ref_genome'])
...
for k1 in os.listdir(dict_opts['-p']):
	if k1.split('.')[-1]=='coverge':
		...
		while True:
			...
			structure1=bp_to_let([pin1])
			structure2=pin2[0]
			if not pin1[0] in in_hash.keys():
				in_hash[pin1[0]]={}
			if not int(pin1[1]) in in_hash[pin1[0]].keys():
				in_hash[pin1[0]][int(pin1[1])]=[]
			if not pin1+[score,structure1,structure2] in in_hash[pin1[0]][int(pin1[1])]:
				in_hash[pin1[0]][int(pin1[1])].append(pin1+[score,structure1,structure2])
		fin.close()
...
write_comp_new(comp_new,dict_opts['-p']+'SVelter.SV.compare.report')

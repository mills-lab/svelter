...
#!python
#command='comp.SV.rec2bed.py -i comp.SV.rec --ref /nfs/remills-scratch/datasets/Simulation.Xuefang/reference.flux/human_g1k_v37.fasta'
...
def hash_reorder():
	...
	for ka1 in tra1.keys():
		ks1=ka1.split('_')[0]
		ks2='_'.join(ka1.split('_')[:-4]+[ka1.split('_')[-1]])
		SV_Score=float(ka1.split('_')[-4])
		Pass_Sign='PASS'
...
for line in fin:
	...
	if not pin[-1] in sv_info[pin[-2]].keys():
		sv_info[pin[-2]][pin[-1]]=[]
	sv_info[pin[-2]][pin[-1]].append(pin)

...
if dict_opts['--server']=='axiom':
        ...
        for k1 in os.listdir(ppre):
                if k1.split('.')[-1]=='rec':
                        ...
                        print >>fo, ' '.join(['Pacbio.Vali.LongDEL.NA12878.Stat.Analysis.py','--bam',bam_NA12878,'--ref',ref,'--ppre',out_path,'-i',ppre+k1])
                        fo.close()
        ...
        for k1 in os.listdir(ppre):
                if k1.split('.')[-1]=='rec':
                        ...
                        out_path=ppre+JobToDo
                        print >>fo, ' '.join(['mkdir',out_path])
                        print >>fo, ' '.join(['Pacbio.Vali.LongDEL.NA12878.py','--bam',bam_NA12878,'--ref',ref,'--ppre',out_path,'-i',ppre+k1])
                        fo.close()

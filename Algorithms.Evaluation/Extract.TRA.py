...
#command='python SVelter.py --ppre /mnt/EXT/Mills-scratch/datasets/Simulation.Xuefang/Simulate.het/bp_files/Simulate.het.RD.20.RL.101.sorted/SPCff4.CluCff6.AlignCff0.0/ -o /mnt/EXT/Mills-scratch/datasets/Simulation.Xuefang/Simulate.het/SVelter/SVelter_het_RD20 --ref /mnt/EXT/Mills-scratch/datasets/Simulation.Xuefang/reference/human_g1k_v37.fasta'
	...
	if k1.split('.')[-1]=='coverge':
		...
		while True:
			...
			if float(pin4[-1])-float(pin3[-1])>qual_cff:
					if not bp_to_let([pin1]) in sv_hash.keys():
						sv_hash[bp_to_let([pin1])]={}
					if not pin2[0] in sv_hash[bp_to_let([pin1])].keys():
						sv_hash[bp_to_let([pin1])][pin2[0]]=[]
					sv_hash[bp_to_let([pin1])][pin2[0]].append(pin1+[float(pin4[-1])-float(pin3[-1])])
		fin.close()
...
sv_out={}
hash_reorder()
output_file=dict_opts['-o']+'.BPlink.vcf'
